...
- **协作模式选型**——判断任务对协作的真实需求形态（迭代精炼？多视角互补？对抗检验？动态拆解？），先于拓扑选型（见"协作模式选型决策程序"）
...
拓扑回答"数据怎么流"，协作意图回答"为什么要多个智能体"。**先定意图，再落拓扑**——因为同一拓扑承载不同意图时，故障模式完全不同：fan-out 承载"互补评审"时，综合者要处理的是视角缺失；承载"对抗辩论"时，要处理的是立场僵持。跳过意图直接选拓扑，最典型的错误是把"需要对抗验证的任务"装进流水线——得到一份自洽、流畅、但带系统性偏差的结论，而且没有人知道它错了。

### 决策程序：按序自问，首个命中即停

从最便宜的模式问起，能被便宜模式解决的任务，禁止用贵模式。

**Q1 — 任务真的需要多个智能体吗？**
判据：产出无需多角色分工、无独立验收需求、上下文单会话可容纳。
命中 → **单智能体**。多智能体是成本，不是目标。demo 里看着气派的六 agent 流水线，往往一个 agent 加两段 prompt 就够了。

**Q2 — 产出有客观可执行的判定标准，且首轮产出预期不完美？**
判据：正确性可以被独立度量（代码 vs 测试、SQL vs 数据对账、schema 合规、数字溯源），而不是"读起来好不好"。
命中 → **评估者-优化器**（Generator + Evaluator 循环）。
纪律：最大迭代数（建议 3）、连续 2 次分数停滞即退出上报、Evaluator 必须用与 Generator 不同的评判框架（否则共享盲点，循环只是原地打转）。

**Q3 — 正确性需要多个不同维度的视角覆盖，视角之间是互补（并集）关系？**
判据：单一视角有盲区，但盲区来自**分工**而非**立场**——法务查合规、财务查成本、技术查可行性，各自检查不同类型的错误，结论不冲突。
命中 → **并行 fan-out + Synthesizer**（专家小组）。
特有故障：部分失败。5 个视角只回来 3 个时，Synthesizer 必须有显式处理策略（接受并降置信度 / 重派缺失分支 / 上报），禁止静默当作"只有 3 个视角的任务"。

**Q4 — 结论存在立场性偏差，错误恰恰藏在"自己检查自己"发现不了的地方？**
判据：这是与 Q3 的分界线。Q3 的视角互补（各查各的维度），Q4 的视角**对立**（同一结论的正反两面）。适用信号：归因结论（"销量降是因为涨价吗"——单一 agent 有确认偏差）、策略权衡（误杀 vs 漏杀的取舍）、高风险决策的最终拍板。共同特征：**没有客观可执行的对错标准**（有的话 Q2 已命中），只有对抗才能暴露的立场性盲点。
命中 → **辩论-裁决**（mesh 的受控用例：正方论证 + 反方攻击 + 裁决者）。
纪律：仅用于错误代价远高于 token 成本的决策节点（成本约 3-4x）；必须有裁决者和终止条件；裁决者对每条质疑给出"成立/不成立 + 理由"，禁止和稀泥；两轮僵持则结论降级为"方向性判断 + 明示局限"，升级人工。

**Q5 — 子任务集合能在设计时预先确定吗？**
能 → **顺序链 / 流水线**（线性依赖时）或 **fan-out**（相互独立时）。
不能（需运行时根据中间结果动态拆解）→ **层级式编排者**。

**Q6 — 真的需要无裁决者的对等协商吗？**
几乎从不需要。默认层级式。通用 mesh 协商仍然默认反对（见模式 5）。

### 成本阶梯（选型约束）

```
单智能体 (1x) < 流水线 (n·x 串行) < fan-out (k·x 并行，墙钟短)
              < 评估者-优化器 (1.5~3x，视迭代数)
              < 专家小组 (k·x + 综合) < 辩论-裁决 (3~4x)
```

规则：自上而下问，命中最便宜的那个。对抗类模式（Q4）是预算中的奢侈品，只买给"错了会出事"的节点。

### 混合是常态

生产级流水线通常是混合体：主干流水线，产出验收节点嵌评估-优化循环，结论决策节点嵌辩论-裁决。设计文档中必须**逐节点写明命中的是哪个 Q、什么模式、为什么**——"整个任务用 mesh"不是设计，是没做选型。

### 走查示例：涨价 SKU 归因分析

- 数据侦察、口径设计 → **Q1 命中**：单 agent 推理任务，不编排
- 取数与指标计算 → **Q5 命中（可预先确定）**：流水线
- 指标验证 → **Q2 命中**：行数对账、抽样重算、单位错位检查——全是有客观标准的判定 → 评估者-优化器，**不是**辩论
- 归因分析（"涨价导致销量降，还是预期销量降而涨价"）→ **Q4 命中**：无客观标准、单一 agent 有确认偏差 → 辩论-裁决
- 报告终审 → **Q2 命中**：数字溯源、图文一致——客观可判 → 评估者-优化器

注意 S 验证和归因的区分：同样是"验证"需求，前者有客观标准（Q2 先命中，走评估循环），后者没有（Q4 才命中，走对抗）。**这就是决策程序的价值——它阻止你把所有"验证"笼统地交给同一个模式。**
...
- **例外：辩论-裁决是对抗验证场景下 mesh 的受控用例**（见"协作模式选型决策程序"Q4）——正方 + 反方 + 裁决者的固定三角色结构，有终止条件和裁决机制，仅限无客观判定标准、立场性偏差风险高的决策节点。这与"开放式对等协商"不同：角色固定、不循环互读全文、裁决者收敛
